...
namespace SeventyOneSquared
{
	...
	public class PDUnity : MonoBehaviour {
		...
		void Initialize()
		{
			...
			this.currentBlendSource = -1;
			this.currentBlendDestination = -1;
		...
		void LateUpdate()
		{
			UpdateWithDelta(Time.smoothDeltaTime);
		}
		...
		void UpdateWithDelta(float aDelta)
		{
			...
			if (!this.Running && emitterConfig.particleCount == 0) {
				if (AutoLoop) {
					this.reset();
				}
			}
		...
		private void parseBinaryFile()
		{
			...
			if (version == 1) {
				...
				// Texture
				emitterConfig.texture = new Texture2D(2, 2, TextureFormat.RGBA32, false);
