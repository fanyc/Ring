...
namespace System.Numerics
{
	...
	public class BigInteger
	{
		...
		public static decimal ToDecimal(BigInteger value)
		{
			...
			return System.Decimal.Parse(value.ToString(), System.Globalization.NumberStyles.Integer, System.Globalization.CultureInfo.CurrentCulture);
		}
		...
		public static BigInteger Pow(BigInteger _base, int exponent)
		{
	        ...
	        if (_base.IsZero == true)
	            return (exponent==0 ? 1 : _base);
			if(exponent==0)
				return 1;
							
	        // Perform exponentiation using repeated squaring trick
	        int newSign = (_base.IsNegative == true && ( exponent & 1 )== 1 ? -1 : 1);
	        BigInteger baseToPow2 = _base;
	        BigInteger result = 1;
	        ...
	        while (exponent != 0) {
	            ...
	            if ((exponent = (int)((uint)exponent >> 1)) != 0) {
	                baseToPow2 *= baseToPow2;
	                //baseToPow2 = trustedStripLeadingZeroInts(baseToPow2);
	            }
	        }
			
	        return Abs(result) * newSign;
		}
